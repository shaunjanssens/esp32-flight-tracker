...
bool Settings::load()
{
    ...
    hide_ground = prefs.getBool("hide_gnd", hide_ground);
    north_offset_deg = prefs.getUShort("north", north_offset_deg);
    display_rotation_deg = prefs.getUShort("rotdeg", display_rotation_deg);
    max_flight_level = prefs.getUShort("maxfl", max_flight_level);
    trail_mode = prefs.getUChar("trails", trail_mode);
    label_count = prefs.getUChar("labels", label_count);
    label_altitude = prefs.getBool("labelalt", label_altitude);
    metric = prefs.getBool("metric", metric);
...
bool Settings::save() const
{
    ...
    prefs.putUShort("north", north_offset_deg);
    prefs.putUShort("rotdeg", display_rotation_deg);
    prefs.putUShort("maxfl", max_flight_level);
    prefs.putUChar("trails", trail_mode);
    prefs.putUChar("labels", label_count);
    prefs.putBool("labelalt", label_altitude);
